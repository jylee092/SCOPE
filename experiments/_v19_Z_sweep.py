...
from pipeline.attack_chain import (
    ...
    apply_minimum_regret_guard, _SEM_SCORER_CACHE,
)
...
def run_viterbi_all(z_margin: float | None):
    ...
    for ds in datasets:
        ...
        if z_margin is not None:
            vit = apply_minimum_regret_guard(
                vit, multi,
                transition_weight=config.VITERBI_TRANSITION_WEIGHT,
                margin_threshold=z_margin,
            )
...
def main():
    ...
    for name, thr in VARIANTS:
        ...
        results.append({"variant": name, "threshold": thr, **m})

    out = config.OUTPUT_BASE_DIR / "v19_Z_sweep_results.json"
