"""Score MAGIC baseline against SCOPE plausibility-based GT (mirrors sigma/score.py)."""
...
MAGIC_BASELINE_DIR = config.OUTPUT_BASE_DIR / "baselines" / "magic"
...
def find_scope_files(result_path: Path) -> tuple[Path | None, Path | None]:
    rel = result_path.relative_to(MAGIC_BASELINE_DIR).parent
...
def score_scenario(result_path: Path, k: int = 5) -> dict | None:
    ...
    alerts = pred.get("notes", {}).get("alerts", [])
...
def main(k: int = 5) -> None:
    paths = sorted(MAGIC_BASELINE_DIR.rglob("result.json"))
    print(f"Found {len(paths)} MAGIC result files")
    rows = [r for r in (score_scenario(p, k=k) for p in paths) if r]
    ...
    n_tp = sum(r["tp_groups"] for r in rows)
    n_hit = sum(r["tp_groups_hit_k"] for r in rows)
    print(f"  TP groups (total) : {n_tp}, micro Hit@{k} = "
          f"{n_hit/n_tp if n_tp else 0:.4f}")

    out = MAGIC_BASELINE_DIR / "_scores.json"
    with open(out, "w", encoding="utf-8") as f:
        json.dump({
            ...
            "micro_hit_k": (n_hit/n_tp) if n_tp else 0,
            "tp_groups_total": n_tp,
            "tp_groups_hit_total": n_hit,
            "rows": rows,
